...
// 执行某个『没有具体返回数据』的『变更』操作成功后，调用  e.g. 删除、修改密码、修改手机号
...
// Data 响应 200 和 带 data 的 JSON 数据
// 执行『更新操作』成功调用后，e.g. 更新话题，成功后返回已更新的话题
...
// Abort404 响应 404，未传参 msg 时，使用默认消息
...
// Abort403 响应 403，未传参 msg 时，使用默认消息
...
// Abort500 响应 500，未传参 msg 时，使用默认消息
...
// BadRequest 响应 400，传参 err 对象，未传参时，使用默认消息
// 在解析用户请求时，如果请求的格式或者方法不符合预期，调用
func BadRequest(c *gin.Context, err error, msg ...string) {
	logger.LogIf(err)

	c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{
		"message": defaultMessage("请求解析错误，请确认格式是否正确。上传文件请使用 multipart 标头，参数请使用 JSON 格式。", msg...),
		"error": err.Error(),
	})
}
...
// Error 响应 404 或 422，未传参 msg 时使用默认信息
// 处理请求时出现 err，会附带返回 error 信息，如登录错误、找不到 ID 对应的 Model
